...
var doodleRig = new function(){
  ...
  that.setup = function(args){
    ...
    FAT = args.fat;
    BLEED = args.bleed;
    DBG_CANVAS_ID = args.debugCanvasId;
    SKEL_CANVAS_ID = args.skeletonCanvasId;
    BLOB_CANVAS_ID = args.blobCanvasId;
    
    skeletonization.setup(WIDTH,HEIGHT);
    
    var canvasIds = [undefined, BLOB_CANVAS_ID, SKEL_CANVAS_ID, DBG_CANVAS_ID];
  ...
  function dbg_render(dbg_context,nodes){
    dbg_context.fillStyle="white";
    dbg_context.fillRect(0,0,WIDTH,HEIGHT);
    dbg_context.save();
      dbg_context.globalCompositeOperation = "multiply"
      dbg_context.save();
        dbg_context.globalAlpha = 0.5;
        dbg_context.drawImage(canvases[1],0,0,WIDTH,HEIGHT);
      dbg_context.restore();

      dbg_context.strokeStyle="red";
      dbg_context.fillStyle = "none";
      dbg_context.lineWidth = 2;
      doodleMeta.drawTree(dbg_context.canvas,nodes[0]);
    dbg_context.restore();  
  }
}
